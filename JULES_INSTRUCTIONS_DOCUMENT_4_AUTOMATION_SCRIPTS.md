...
```python
"""
Shared colors module for terminal output
"""
...
try:
    ...
    # If run from root as a module or in some other way, try relative import
    try:
        from .colors import Colors
    except ImportError:
         # Last resort, if package structure is different
...
Save the Python files above as:
- `scripts/colors.py`
