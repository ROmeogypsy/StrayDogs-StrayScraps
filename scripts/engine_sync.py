...
def parse_canon_update(session_file: Path) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        logger.error(f"{Colors.RED}Error parsing {session_file}: {e}{Colors.RESET}")
        return None
...
def find_character_file(char_name: str) -> Optional[Path]:
    ...
    logger.warning(f"{Colors.YELLOW}Warning: Character file not found for '{char_name}'{Colors.RESET}")
    return None
...
def update_character(char_name: str, changes: Dict[str, Any], session_id: str) -> bool:
    ...
    try:
        ...
        logger.info(f"{Colors.GREEN}✓ Updated {char_name}{Colors.RESET}")
        return True

    except Exception as e:
        logger.error(f"{Colors.RED}Error updating {char_name}: {e}{Colors.RESET}")
        return False
...
def update_tension(tension_name: str, changes: Dict[str, Any], session_id: str) -> bool:
    ...
    if not tension_file.exists():
        logger.warning(f"{Colors.YELLOW}Warning: Tension file not found for '{tension_name}'{Colors.RESET}")
        return False
    ...
    try:
        ...
        logger.info(f"{Colors.GREEN}✓ Updated tension: {tension_name}{Colors.RESET}")
        return True

    except Exception as e:
        logger.error(f"{Colors.RED}Error updating tension {tension_name}: {e}{Colors.RESET}")
        return False

def apply_canon_updates(canon_data: Dict[str, Any], session_id: str) -> int:
    ...
    if 'new_events' in canon_data:
        for event in canon_data['new_events']:
            ...
            logger.info(f"{Colors.BLUE}→ New event logged: {event.get('title', 'Unnamed Event')}{Colors.RESET}")
            updates_applied += 1
...
def git_commit(session_id: str, summary: str):
    ...
    try:
        ...
        logger.info(f"{Colors.GREEN}✓ Git commit created{Colors.RESET}")

    except Exception as e:
        logger.error(f"{Colors.RED}Git commit failed: {e}{Colors.RESET}")

def process_session_file(session_file: Path):
    """Process a single session file"""
    logger.info(f"\n{Colors.BLUE}{'='*60}{Colors.RESET}")
    logger.info(f"{Colors.BLUE}Processing: {session_file.name}{Colors.RESET}")
    logger.info(f"{Colors.BLUE}{'='*60}{Colors.RESET}")

    canon_data = parse_canon_update(session_file)

    if not canon_data:
        logger.warning(f"{Colors.YELLOW}No CANON_UPDATE block found{Colors.RESET}")
        return
...
def watch_sessions():
    ...
    logger.info(f"\n{Colors.BLUE}{'='*60}{Colors.RESET}")
    logger.info(f"{Colors.BLUE}STRAY DOGS WORLDENGINE - ENGINE SYNC{Colors.RESET}")
    logger.info(f"{Colors.BLUE}Monitoring: {SESSIONS_DIR}{Colors.RESET}")
    logger.info(f"{Colors.BLUE}{'='*60}{Colors.RESET}\n")
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if session_file.exists():
            ...
            logger.error(f"{Colors.RED}File not found: {session_file}{Colors.RESET}")
    else:
        # Watch mode
        watch_sessions()
